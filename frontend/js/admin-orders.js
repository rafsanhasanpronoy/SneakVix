...
function readControlsToState() {
  return {
    ...
    ordering: document.getElementById("orderSort").value,
  };
}
...
async function loadOrders(pushHistory = false) {
  ...
  el.innerHTML = `<tr><td colspan="7" class="empty-row">Loading...</td></tr>`;
  ...
  try {
    ...
    if (orders.length === 0) {
      el.innerHTML = `<tr><td colspan="7" class="empty-row">No orders match.</td></tr>`;
      return;
    }
    ...
    el.querySelectorAll(".status-select").forEach((sel) => {
      sel.addEventListener("change", async () => {
        sel.disabled = true;
        try {
          await api.patch(`/admin/orders/${sel.dataset.id}/`, { status: sel.value });
          loadOrders();
        } catch {
          sel.disabled = false;
        }
    ...
    el.innerHTML = `<tr><td colspan="7" class="empty-row">Could not load orders.</td></tr>`;
  }
}
